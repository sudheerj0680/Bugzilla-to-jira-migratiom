...
BUGZILLA_SPACE = ENV['BUGZILLA_SPACE'].freeze
...
JIRA_API_BROWSE_ISSUE = ENV['JIRA_API_BROWSE_ISSUE'] || 'browse/[:issue_id]'
JIRA_API_BROWSE_COMMENT = ENV['JIRA_API_BROWSE_COMMENT'] || 'browse/[:issue_id]?focusedCommentId=[:jira-comment-id]&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-[:jira-comment-id]'
...
#OUTPUT_DIR_BUGZILLA = output_dir_bugzilla(BUGZILLA_SPACE)
OUTPUT_DIR_JIRA = output_dir_jira(BUGZILLA_SPACE)
...
[OUTPUT_DIR, OUTPUT_DIR_JIRA_ATTACHMENTS].each do |dir|
...
JIRA_HEADERS_ADMIN = { 'Authorization': "Basic #{base64_admin}", 'Content-Type': 'application/json; charset=utf-8', 'Accept': 'application/json' }.freeze

# Assuming that the user name is the same as the user password
# For the cloud we use the email otherwise login
def headers_user_login(issue_id, who)
  return JIRA_HEADERS_ADMIN if JIRA_SERVER_TYPE == 'cloud'
  {'Authorization': "Basic #{Base64.encode64(who + ':' + who)}", 'Content-Type': 'application/json; charset=utf-8'}
end
...
def http_request(url, opts = {})
  ...
  begin
    response = RestClient::Request.execute(method: :get, url: url, headers: BUGZILLA_HEADERS)
...
def bugzilla_get_spaces
  response = http_request("#{BUGZILLA_API_HOST}/spaces")
...
def get_space(name)
  spaces = csv_to_array("#{OUTPUT_DIR_BUGZILLA}/spaces.csv")
...
# TODO: Similar to get_ticket_attr() in 02-bugzilla_export_tickets.rb (refactor)
def get_items(items, space)
  items.each do |item|
    url = "#{BUGZILLA_API_HOST}/spaces/#{space['id']}/#{item[:name]}"
    if item[:q]
      url += "?#{item[:q]}"
...
def create_csv_file(space, item)
  dirname = get_output_dirname(space, 'bugzilla')
  filename = "#{dirname}/#{normalize_name(item[:name])}.csv"
  write_csv_file(filename, item[:results])
end

def export_bugzilla_items(list)
  space = get_space(BUGZILLA_SPACE)
...
def jira_create_user(user)
  ...
  begin
    ...
    puts "POST #{url} username='#{email}' => OK (#{body.to_json})"
    result = body
  rescue RestClient::ExceptionWithResponse => e
    if e.class == RestClient::InternalServerError
      puts "POST #{url} username='#{email}' => NOK (#{e}) please retry"
    end
    error = JSON.parse(e.response)
...
def reformat_markdown(content, opts = {})
  ...
  lines.each do |line|
    ...
    markdown << line.
        ...
        gsub(/\[\[image:(.*?)(\|(.*?))?\]\]/i) {|image| markdown_image(image, images, comment)}
  end
  markdown.join("\n")
end
...
def bugzilla_estimate_to_jira_size(estimate)
